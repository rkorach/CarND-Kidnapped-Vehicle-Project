...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[], 
    ...
    for (int j=0; j<obs_trans.size(); ++j){
    	for (int k=0; k < map_landmarks.landmark_list.size(); ++k) {
        	...
        	prob *= exp(-(1/2.0) * ( pow(obs_trans[j].x - ldmk.x_f, 2) / pow(std_landmark[0], 2) + pow(obs_trans[j].y - ldmk.y_f, 2) / pow(std_landmark[1], 2) ));
        	prob = prob/sqrt(2.0 * M_PI * (pow(std_landmark[0], 2) + pow(std_landmark[1], 2))); 
        	if (prob == 0.0) {
